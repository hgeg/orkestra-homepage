<?php
/**
 * Taxonomy Template for the Project Tags
 */
?>
...
if ( $post ) :
	...
	$t = wp_get_post_terms( $post->ID , array('project_tag') );
	$tag = $t[0];
...
?>
	...
	<section id="content" class="portfolio one-column" role="main">
		...
		<header class="page-header">
			...
			<h1 class="page-title"><?php _e( 'Current Tag:', 'framework' ); ?> <span><?php echo( $term->name ); ?></span></h1>

			<?php if ($tag->description) { ?><p class="portfolio-header-description"><?php echo( $tag->description ); ?></p><?php } ?>
