...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
	    ...
	    $mav_data = get_option(OPTIONS)?get_option(OPTIONS):mav_framework_defaults();
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					$t_value = stripslashes($mav_data[$value['id']]);
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {			
						$output .= '<option id="' . $select_ID . '" value="'.$option.'" ' . selected($mav_data[$value['id']], $option, false) . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($mav_data[$value['id']]);			
				...
				case "radio":
					 ...
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($mav_data[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				
				//checkbox option
				case 'checkbox':
					if (!isset($mav_data[$value['id']])) {
						$mav_data[$value['id']] = 0;
					}
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($mav_data[$value['id']], 1, false) .' />';
				break;
				
				//multiple checkbox option
				case 'multicheck': 			
					$multi_stored = $mav_data[$value['id']];
				...
				//colorpicker option
				case 'color':		
					$output .= '<div id="' . $value['id'] . '_picker" class="colorSelector"><div style="background-color: '.$mav_data[$value['id']].'"></div></div>';
					$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'. $mav_data[$value['id']] .'" />';
				break;
				
				//typography option	
				case 'typography':
				
					$typography_stored = isset($mav_data[$value['id']]) ? $mav_data[$value['id']] : $value['std'];
				...
				case 'border':
					...
					/* Border Width */
					$border_stored = $mav_data[$value['id']];
				...
				case 'images':
					...
					$select_value = $mav_data[$value['id']];
				...
				case 'sorter':
					...
					$sortlists = isset($mav_data[$value['id']]) && !empty($mav_data[$value['id']]) ? $mav_data[$value['id']] : $value['std'];
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = isset($mav_data[$value['id']]) && !empty($mav_data[$value['id']]) ? $mav_data[$value['id']] : '';
				...
				case 'backup':
					...
					$output .= '<a href="#" id="of_backup_button" class="button" title="Backup Options">Backup</a>';
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<textarea id="export_data" rows="8">'.base64_encode(serialize($mav_data)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order,$int){
		...
		$temp = get_option(OPTIONS);
		$mav_data = is_array($temp[$id])?get_option(OPTIONS):mav_framework_defaults();
		...
		// initialize all vars
		$slidevars = array('title','url','link','link_behaviour','description');
		...
		// Added some validation to links (eg. for www.domain.tld => the links pointed to the blog itself ... :~ )
		...
		$slider .= '<small style="color:#8f969c;display:block;margin-bottom:10px">Example: http://google.com</small>';

		$slider .= '<label>Link Behaviour</label>';
		$slider .= '<div class="controls"><div class="select_wrapper">';
		$slider .= '<select class="select slide of-input" name="'. $id .'['.$order.'][link_behaviour]" id="'. $id .'_'.$order .'_slide_link_behaviour">';
		$slider .= $val['link_behaviour'] == 'New Window' ? '<option selected="selected">New Window</option>' : '<option>New Window</option>';
		$slider .= $val['link_behaviour'] == 'Same Window' ? '<option selected="selected">Same Window</option>' : '<option>Same Window</option>';
		$slider .= '</select>';
		$slider .= '</div></div><br/>';
		$slider .= '<small style="color:#8f969c;float:left;margin-bottom:10px">How to open this slide link: in a new window or in the same one the slider is in.</small><br/>';
