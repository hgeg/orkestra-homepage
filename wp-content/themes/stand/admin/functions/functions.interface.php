...
function optionsframework_add_admin() {
	...
	// $of_page = add_menu_page( THEMENAME, THEMENAME, 'edit_theme_options', 'optionsframework', 'optionsframework_options_page', get_template_directory_uri() . '/images/mav-icon.png', 99 );
...
function of_ajax_callback() 
{
	...
	elseif($save_type == 'restore_options')
	{
		...
		$mav_data = get_option(BACKUPS);
		
		update_option(OPTIONS, $mav_data);
	...
	elseif($save_type == 'import_options'){
		...
		$mav_data = $_POST['data'];
		$mav_data = unserialize(base64_decode($mav_data)); //100% safe - ignore theme check nag
		update_option(OPTIONS, $mav_data);
	...
	{
		wp_parse_str(stripslashes($_POST['data']), $mav_data);
		unset($mav_data['security']);
		unset($mav_data['of_save']);
		update_option(OPTIONS, $mav_data);
